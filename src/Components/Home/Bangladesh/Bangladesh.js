import React from 'react';
import { Link } from 'react-router-dom';
import '../Bangladesh/Bangladesh.css';
import '../Home.css'
const Bangladesh = () => {
    return (
        <div className='title container'>
            <div className='col-md-12 '>
                <h6><Link to='/' className='font'>
                    <span className='box'></span>
                    বাংলাদেশ</Link>
                </h6>
            </div>
            <div className='DBangTopNews'>

                <div className='row '>
                    <div className='col-md-6 left-lead col-12 border-end'>
                        <div className='lead'>
                            <Link to='/'>
                                <div className='zoom '>
                                    <div className='imgContainer'>
                                        <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Untitled-1-samakal-63bac1fe6c990.webp" alt="" title='...' className='fix-ratio' />
                                    </div>
                                </div>

                            </Link>
                        </div>
                        <Link to='/'>
                            <div className='Desc'>
                                <p className='Title fw-bold'>ব্রাজিলের সঙ্গে বাণিজ্যিক সহযোগিতা বাড়ানোর আহ্বান প্রধানমন্ত্রীর</p>
                                <div className='Brief'>
                                    <p>প্রধানমন্ত্রী শেখ হাসিনা বাংলাদেশ ও ব্রাজিলের মধ্যে বাণিজ্যিক সহযোগিতা বৃদ্ধির প্রয়োজনীয়তার ওপর গুরুত্বারোপ করে বলেছেন, দুই দেশের মধ্যে ব্যবসা সম্প্রসারণের বিপুল সম্ভাবনা রয়েছে।</p>
                                </div>
                            </div>
                        </Link>
                    </div>
                    <div className='col-md-2 left-lead col-12 border-end'>
                        <div className=''>
                            <Link to='/'>
                                <div className='lead '>

                                    <div className='zoom '>
                                        <div className='imgContainer'>
                                            <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Parliament-Adhibession-samakal-63babf7dba9bd.jpg" alt="" title='...' className='fix-ratio' />
                                        </div>
                                    </div>

                                </div>
                                <div className=' border-bottom mb-3'>
                                    <h4 className='pb-2 Title'>রাজধানীতে অনুমোদনহীন ক্লিনিক নেই</h4>

                                </div>
                            </Link>
                        </div>
                        <Link to='/'>
                            <div className='lead '>

                                <div className='zoom '>
                                    <div className='imgContainer'>
                                        <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Parliament-Adhibession-samakal-63babf7dba9bd.jpg" alt=""title='...' className='fix-ratio' />
                                    </div>
                                </div>


                            </div>

                           
                                <h3 className='Title'>রাজধানীতে অনুমোদনহীন ক্লিনিক নেই</h3>
                            
                        </Link>

                    </div>
                    {/* Side part */}
                    <div className='col-md-4 col-12'>
                        <div className='col-md-12'>
                            <div className='row '>
                                <div className='border-bottom d-flex'>
                                    <Link to='#' className='font'>
                                        <div className='col-md-12 col-12 '>
                                            <div className='Bd-sub-list'>
                                                <div className='row'>
                                                    <div className='col-md-8 col-7 '>
                                                        
                                                            <h3 className='Title'>ভারতে কয়লা খনি ধসে ১০ জন আটকে পড়ার শঙ্কা</h3>
                                                       
                                                    </div>
                                                    <div className='col-md-4 col-5'>
                                                        <div className='B-lead'>

                                                            <div className='zoom '>
                                                                <div className='imgContainer'>
                                                                    <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Obaidur-Kader--samakal-63b96d5d964f9.jpg" alt="" title='...' className='fix-ratio' />
                                                                </div>
                                                            </div>


                                                        </div>
                                                    </div>

                                                </div>

                                            </div>

                                        </div>
                                    </Link>
                                </div>
                                <div className='border-bottom d-flex'>
                                    <div className='col-md-12 col-12 '>
                                        <Link to='/' className='font'>
                                            <div className='Bd-sub-list'>
                                                <div className='row'>
                                                    <div className='col-md-8 col-7 '>
                                                        
                                                            <h3 className='Title'>ভারতে কয়লা খনি ধসে ১০ জন আটকে পড়ার শঙ্কা</h3>
                                                        
                                                    </div>
                                                    <div className='col-md-4 col-5'>
                                                        <div className='B-lead'>
                                                            <div className='zoom '>
                                                                <div className='imgContainer'>
                                                                    <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Obaidur-Kader--samakal-63b96d5d964f9.jpg" alt="" className='fix-ratio' title='...' />
                                                                </div>
                                                            </div>


                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </Link>
                                    </div>
                                </div>
                                <div className='border-bottom d-flex'>
                                    <div className='col-md-12 col-12 '>
                                        <Link className='font' to='#'>
                                            <div className='Bd-sub-list'>
                                                <div className='row'>
                                                    <div className='col-md-8 col-7 '>
                                                       
                                                            <h3 className='Title'>ভারতে কয়লা খনি ধসে ১০ জন আটকে পড়ার শঙ্কা</h3>
                                                      
                                                    </div>
                                                    <div className='col-md-4 col-5'>
                                                        <div className='B-lead'>
                                                            <div className='zoom '>
                                                                <div className='imgContainer'>
                                                                    <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Obaidur-Kader--samakal-63b96d5d964f9.jpg" alt="" className='fix-ratio' title='...' />
                                                                </div>
                                                            </div>


                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </Link>
                                    </div>
                                </div>

                                <div className='col-md-12 col-12 '>
                                    <Link className='font' to='#'>
                                        <div className='Bd-sub-list'>
                                            <div className='row'>
                                                <div className='col-md-8 col-7 '>
                                                   
                                                        <h3 className='Title'>ভারতে কয়লা খনি ধসে ১০ জন আটকে পড়ার শঙ্কা</h3>
                                                   
                                                </div>
                                                <div className='col-md-4 col-5'>
                                                    <div className='B-lead'>
                                                        <div className='zoom '>
                                                            <div className='imgContainer'>
                                                                <img src="https://www.emythmaker.com/project/Template/NewsProtal/DailySamakal/Demo-3/media/imgAll/Obaidur-Kader--samakal-63b96d5d964f9.jpg" alt="" className='fix-ratio' title='...' />
                                                            </div>
                                                        </div>


                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </Link>
                                </div>


                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Bangladesh;